...
const UNDO_PICK_CATEGORIES: UndoPickReasonCategory[] = [
  {
    ...
    reasons: [
      { id: 'damaged', label: 'Damaged Item', description: 'Item arrived damaged during picking', requiresNotes: false },
      { id: 'defective', label: 'Defective Item', description: 'Item has quality defect or issue', requiresNotes: true },
      { id: 'expired', label: 'Expired Item', description: 'Item is past expiration date', requiresNotes: false },
    ],
  },
  {
    id: 'wrong_item',
    ...
    reasons: [
      { id: 'wrong_item_picked', label: 'Wrong Item Picked', description: 'Accidentally picked incorrect item', requiresNotes: false },
      { id: 'wrong_bin', label: 'Wrong Bin Location', description: 'Item was from wrong bin location', requiresNotes: true },
      { id: 'wrong_barcode', label: 'Wrong Barcode Scan', description: 'Scanned wrong barcode by mistake', requiresNotes: false },
    ],
  },
  {
    id: 'quantity',
    ...
    color: 'text-info-400',
    reasons: [
      { id: 'overpicked', label: 'Overpicked', description: 'Picked more than required quantity', requiresNotes: false },
      { id: 'adjust_quantity', label: 'Adjust Quantity', description: 'Need to adjust picked quantity', requiresNotes: true },
    ],
  },
  {
    id: 'equipment',
    ...
    reasons: [
      { id: 'scanner_error', label: 'Scanner Error', description: 'Scanner malfunction or error', requiresNotes: false },
      { id: 'system_error', label: 'System Error', description: 'System or app error during pick', requiresNotes: true },
    ],
  },
  {
    id: 'other',
...
export function UndoPickModal({
  ...
  const selectedReasonData = UNDO_PICK_CATEGORIES
        ...
        <div className="inline-block align-bottom glass-card rounded-2xl text-left overflow-hidden shadow-premium-lg transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full animate-scale-in">
          ...
          <div className="bg-white/[0.02] px-6 py-4 sm:px-6 flex items-center justify-between border-b border-white/[0.08]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">{itemName} ({sku})</p>
              </div>
            </div>
            <button
              onClick={handleClose}
          ...
          <div className="px-6 py-4 bg-warning-500/5 border-b border-warning-500/10">
            <p className="text-sm text-warning-300">
              <strong>Warning:</strong> You are about to undo a pick. This will reduce the picked quantity from{' '}
              <span className="font-mono">{currentQuantity}</span> to <span className="font-mono">{newQuantity}</span>.
              ...
              <div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {UNDO_PICK_CATEGORIES.find(c => c.id === selectedCategory)?.reasons.map(reason => (
                      <button
                        key={reason.id}
