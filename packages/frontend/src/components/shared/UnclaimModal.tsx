...
const UNCLAIM_CATEGORIES: UnclaimReasonCategory[] = [
  {
    ...
    reasons: [
      { id: 'scanner_malfunction', label: 'Scanner Malfunction', description: 'Barcode scanner not working', requiresNotes: false },
      { id: 'pallet_jack_broken', label: 'Pallet Jack Broken', description: 'Equipment unavailable or broken', requiresNotes: false },
      { id: 'label_printer_down', label: 'Label Printer Down', description: 'Cannot print shipping labels', requiresNotes: false },
      { id: 'other_equipment', label: 'Other Equipment', description: 'Other equipment issue', requiresNotes: true },
    ],
  },
  {
    id: 'staff',
    ...
    color: 'text-info-400',
    reasons: [
      { id: 'illness', label: 'Illness', description: 'Feeling unwell and unable to continue', requiresNotes: false },
      { id: 'emergency', label: 'Emergency', description: 'Personal emergency requires leaving', requiresNotes: false },
      { id: 'injury', label: 'Injury', description: 'Minor injury preventing work', requiresNotes: false },
    ],
  },
  {
    id: 'order',
    ...
    reasons: [
      { id: 'wrong_items', label: 'Wrong Items', description: 'Order has incorrect items', requiresNotes: true },
      { id: 'damaged_items', label: 'Damaged Items', description: 'Items arrived damaged', requiresNotes: true },
      { id: 'missing_items', label: 'Missing Items', description: 'Cannot find items in order', requiresNotes: true },
      { id: 'complex_order', label: 'Complex Order', description: 'Order too complex for current capacity', requiresNotes: true },
    ],
  },
  {
    id: 'inventory',
    ...
    reasons: [
      { id: 'cannot_locate', label: 'Cannot Locate', description: 'Unable to find item location', requiresNotes: true },
      { id: 'bin_empty', label: 'Bin Empty', description: 'Bin shows stock but is actually empty', requiresNotes: true },
      { id: 'wrong_bin', label: 'Wrong Bin Location', description: 'Item is in different bin than system shows', requiresNotes: true },
      { id: 'stock_discrepancy', label: 'Stock Discrepancy', description: 'System shows different quantity than actual', requiresNotes: true },
    ],
  },
  {
    id: 'other',
    ...
    color: 'text-gray-400',
    reasons: [
      { id: 'other', label: 'Other Reason', description: 'Another reason not listed above', requiresNotes: true },
    ],
  },
];
...
export function UnclaimModal({
  ...
  const selectedReasonData = UNCLAIM_CATEGORIES
    .flatMap(cat => cat.reasons)
    .find(r => r.id === selectedReason);

  const canConfirm = selectedReason && (!selectedReasonData?.requiresNotes || notes.trim().length > 0);
        ...
        <div className="inline-block align-bottom glass-card rounded-2xl text-left overflow-hidden shadow-premium-lg transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full animate-scale-in">
          ...
          <div className="px-6 py-4 bg-error-500/5 border-b border-error-500/10">
            <p className="text-sm text-error-300">
              <strong>Warning:</strong> You are about to unclaim this order. All your progress will be reset
              and the order will return to the queue. This action cannot be undone.
            </p>
          </div>
                  ...
                  {UNCLAIM_CATEGORIES.map(category => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-400 mt-2">{category.reasons.length} options</p>
                      </button>
                    );
                  })}
                </div>
              ...
              <div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {UNCLAIM_CATEGORIES.find(c => c.id === selectedCategory)?.reasons.map(reason => (
                      <button
                        key={reason.id}
                      ...
                      >
                        ...
                        {reason.requiresNotes && (
                          <div className="text-xs text-warning-400 mt-2">Additional notes required</div>
                        )}
                      </button>
                    ))}
                  </div>
