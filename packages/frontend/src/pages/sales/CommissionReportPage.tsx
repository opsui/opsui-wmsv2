      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Commission Reports</h1>
              <p className="text-slate-400 text-sm">View and manage sales commissions</p>
            </div>
          </div>
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card title="Total Earned" className="bg-slate-800/50">
            <p className="text-3xl font-bold text-blue-400">
              ${totals.totalEarned.toLocaleString()}
            </p>
          </Card>
          <Card title="Total Paid" className="bg-slate-800/50">
            <p className="text-3xl font-bold text-emerald-400">
              ${totals.totalPaid.toLocaleString()}
            </p>
          </Card>
          <Card title="Pending Payment" className="bg-slate-800/50">
            <p className="text-3xl font-bold text-amber-400">
              ${totals.pendingPayment.toLocaleString()}
            </p>
          </Card>
          <Card title="Transactions" className="bg-slate-800/50">
            <p className="text-3xl font-bold text-purple-400">{totals.transactionCount}</p>
          </Card>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Commission Summary by Sales Person */}
          <Card title="Commission Summary by Sales Person" className="lg:col-span-1">
            {loading ? (
                  ...
                  <tbody>
                    {summaries.map(summary => (
                      <tr
                        ...
                        className={`border-b border-slate-700/30 cursor-pointer hover:bg-slate-800/30 ${
                          selectedSalesPerson === summary.salesPersonId ? 'bg-blue-500/10' : ''
                        }`}
          ...
          {/* Commission Details */}
          {showDetails && (
            <Card title="Commission Details" className="lg:col-span-1">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {details.map(detail => (
                      <tr key={detail.commissionId} className="border-b border-slate-700/30">
                        <td className="py-2">
                          <Link
                        ...
                        <td className="py-2 text-right text-slate-300 text-sm">
                          ${detail.baseAmount.toLocaleString()}
                        </td>
                        <td className="py-2 text-right text-slate-400 text-sm">
                          {detail.commissionRate}%
                        </td>
                        <td className="py-2 text-right text-white font-semibold text-sm">
                  ...
                  return (
                    <div key={summary.salesPersonId}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-white text-sm">{summary.salesPersonName}</span>
