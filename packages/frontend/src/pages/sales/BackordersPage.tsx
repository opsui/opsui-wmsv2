...
export default function BackordersPage() {
  ...
  const getPriorityColor = (priority: number) => {
    if (priority <= 2) return 'text-red-500 dark:text-red-400';
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Card className="bg-white dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700">
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 dark:text-slate-400">No backorders found</p>
            </div>
          ) : (
                  ...
                  {backorders.map(backorder => (
                    <tr
                      ...
                      className={`border-b border-gray-100 dark:border-slate-700/30 hover:bg-gray-50 dark:hover:bg-slate-800/30 ${
                        isOverdue(backorder.promisedDate) ? 'bg-red-50 dark:bg-red-500/5' : ''
                      }`}
                    >
                      <td className="py-3">
                        <Link
                      ...
                      <td className="py-3">
                        <Link
                          ...
                          className="text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-300 text-sm"
                        >
                      ...
                      <td className="py-3 text-right">
                        ...
                        {backorder.status === 'FULFILLED' && (
                          <span className="px-3 py-1.5 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-lg text-sm font-medium inline-flex items-center gap-1">
                            <CheckIcon className="h-4 w-4" />
                            Complete
                          </span>
                        )}
        ...
        {backorders.some(b => isOverdue(b.promisedDate)) && (
          <Card className="mt-6 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30">
