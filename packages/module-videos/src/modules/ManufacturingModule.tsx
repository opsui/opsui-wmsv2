...
const MainScene = () => {
  ...
  const opacity = interpolate(frame, [0, fps * 0.3], [0, 1], { extrapolateRight: 'clamp' });
  const jobs = [
    { id: 'WO-001', product: 'Widget Assembly A', progress: 75, status: 'running' },
    { id: 'WO-002', product: 'Component Kit B', progress: 100, status: 'complete' },
    { id: 'WO-003', product: 'Custom Module C', progress: 20, status: 'pending' },
  ];
  ...
  return (
    <div style={{ opacity, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        {[
          { label: 'Active Jobs', value: 12, color: colors.accent },
          { label: 'Completed', value: 89, color: colors.success },
          { label: 'Efficiency', value: '94%', color: colors.warning },
        ].map((m, i) => (
...
const OutroScene = () => {
  ...
  return (
    ...
    >
      ...
      >
        <ValueBadge text="Work Orders" delay={5} />
        <ValueBadge text="Bill of Materials" delay={10} />
        <ValueBadge text="Quality Control" delay={15} />
      </div>
...
export const ManufacturingModule = ({ accentColor }: { accentColor: string }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  return (
