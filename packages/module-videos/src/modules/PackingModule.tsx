...
const IntroScene = () => {
  ...
  return (
    <div style={{ textAlign: 'center', opacity, transform: `translateY(${translateY}px)` }}>
      ...
      >
        Streamlined packing with integrated verification
      </p>
    </div>
  );
};
...
const MainScene = () => {
  ...
  const opacity = interpolate(frame, [0, fps * 0.3], [0, 1], { extrapolateRight: 'clamp' });
  const orders = [
    { id: 'ORD-001', items: 3, status: 'packing' },
    { id: 'ORD-002', items: 5, status: 'pending' },
    { id: 'ORD-003', items: 2, status: 'complete' },
  ];
  ...
  return (
    <div style={{ opacity, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        {[
          { label: 'Packed Today', value: 47, color: colors.success },
          { label: 'Queue', value: 12, color: colors.warning },
          { label: 'Avg Time', value: '3m', color: colors.accent },
        ].map((m, i) => (
...
const OutroScene = () => {
  ...
  return (
    ...
    >
      ...
      >
        <ValueBadge text="Weight Verification" delay={5} />
        <ValueBadge text="Auto-Print Labels" delay={10} />
        <ValueBadge text="Photo Capture" delay={15} />
      </div>
...
export const PackingModule = ({ accentColor }: { accentColor: string }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  return (
