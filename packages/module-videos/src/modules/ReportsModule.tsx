...
const MainScene = () => {
  ...
  const opacity = interpolate(frame, [0, fps * 0.3], [0, 1], { extrapolateRight: 'clamp' });
  const reports = [
    { name: 'Daily Operations', icon: '📊' },
    { name: 'Pick Efficiency', icon: '⚡' },
    { name: 'Stock Levels', icon: '📦' },
    { name: 'Shipping Stats', icon: '🚚' },
  ];
  ...
  return (
    <div style={{ opacity, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        {[
          { label: 'Reports Run', value: '1.2K', color: colors.accent },
          { label: 'Scheduled', value: 23, color: colors.success },
...
const OutroScene = () => {
  ...
  return (
    ...
    >
      ...
      >
        <ValueBadge text="Custom Dashboards" delay={5} />
        <ValueBadge text="Export Anywhere" delay={10} />
        <ValueBadge text="Scheduled Reports" delay={15} />
      </div>
...
export const ReportsModule = ({ accentColor }: { accentColor: string }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  return (
