...
const IntroScene = () => {
  ...
  return (
    <div style={{ textAlign: 'center', opacity, transform: `translateY(${translateY}px)` }}>
      ...
      >
        Automated workflows that drive productivity
      </p>
    </div>
  );
};
...
const MainScene = () => {
  ...
  const opacity = interpolate(frame, [0, fps * 0.3], [0, 1], { extrapolateRight: 'clamp' });
  const tasks = [
    { name: 'Order Verification', status: 'complete', time: '2m' },
    { name: 'Inventory Allocation', status: 'complete', time: '1m' },
    { name: 'Pick List Generation', status: 'running', time: '—' },
    { name: 'Carrier Selection', status: 'pending', time: '—' },
  ];
  ...
  return (
    <div style={{ opacity, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        {[
          { label: 'Active Tasks', value: 24, color: colors.accent },
          { label: 'Completed', value: 156, color: colors.success },
          { label: 'Avg Time', value: '4m', color: colors.warning },
        ].map((m, i) => (
          ...
          >
            <div
              style={{
                ...
                alignItems: 'center',
                justifyContent: 'center',
                border: `2px solid ${task.status === 'complete' ? colors.success : task.status === 'running' ? colors.accent : 'rgba(255,255,255,0.2)'}`,
              }}
            >
              {task.status === 'complete' && (
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 12, color: colors.textPrimary, margin: 0 }}>{task.name}</p>
            </div>
            <span
              style={{
                fontFamily: 'JetBrains Mono, monospace',
...
const OutroScene = () => {
  ...
  return (
    ...
    >
      ...
      >
        <ValueBadge text="Custom Workflows" delay={5} />
        <ValueBadge text="Event Triggers" delay={10} />
        <ValueBadge text="Real-time Alerts" delay={15} />
      </div>
...
export const TaskExecutionModule = ({ accentColor }: { accentColor: string }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  return (
