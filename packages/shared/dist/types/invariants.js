 ...
 */
export function invariantInventoryNeverNegative(quantity) {
    if (quantity < 0) {
        throw new Error(`INVARIANT VIOLATION: Inventory quantity is negative (${quantity}). ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantReservedNeverExceedsTotal(quantity, reserved) {
    if (reserved > quantity) {
        throw new Error(`INVARIANT VIOLATION: Reserved quantity (${reserved}) exceeds total quantity (${quantity}). ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantProgressRange(progress) {
    if (progress < 0 || progress > 100) {
        throw new Error(`INVARIANT VIOLATION: Order progress (${progress}) is outside valid range [0, 100]. ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantPickedNeverExceedsOrdered(quantity, pickedQuantity) {
    if (pickedQuantity > quantity) {
        throw new Error(`INVARIANT VIOLATION: Picked quantity (${pickedQuantity}) exceeds ordered quantity (${quantity}). ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantQuantityAlwaysPositive(quantity) {
    if (quantity <= 0) {
        throw new Error(`INVARIANT VIOLATION: Quantity (${quantity}) is not positive. ` +
 ...
 */
export function invariantTerminalStateImmutable(status) {
    const terminalStates = [OrderStatus.SHIPPED, OrderStatus.CANCELLED];
    if (terminalStates.includes(status)) {
        throw new Error(`INVARIANT VIOLATION: Attempting to transition from terminal state (${status}). ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantPickerRequiredForPickingStates(status, pickerId) {
    ...
    if (pickerRequiredStates.includes(status) && pickerId === undefined) {
        throw new Error(`INVARIANT VIOLATION: Order is in ${status} state but no picker is assigned. ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantPackerRequiredForPackingStates(status, packerId) {
    ...
    if (packerRequiredStates.includes(status) && packerId === undefined) {
        throw new Error(`INVARIANT VIOLATION: Order is in ${status} state but no packer is assigned. ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantBinLocationFormatValid(binId) {
    const pattern = /^[A-Z]-\d{1,3}-\d{2}$/;
    if (!pattern.test(binId)) {
        throw new Error(`INVARIANT VIOLATION: Bin location ID "${binId}" has invalid format. ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantReservationsReleasedOnCancellation(orderId, remainingReservations) {
    if (remainingReservations > 0) {
        throw new Error(`INVARIANT VIOLATION: Order ${orderId} was cancelled but ${remainingReservations} reservations remain. ` +
 ...
 */
export function invariantAuditTrailImmutable() {
    throw new Error(`INVARIANT VIOLATION: Attempt to delete audit trail entries. ` +
 ...
 */
export function invariantForeignKeysNeverDisabled() {
    throw new Error(`INVARIANT VIOLATION: Attempt to disable foreign key constraints. ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantOrderIdUnique(existingOrder, orderId) {
    if (existingOrder !== undefined && existingOrder !== null) {
        throw new Error(`INVARIANT VIOLATION: Order ID ${orderId} already exists. ` +
 ...
 * @throws Error if invariant is violated
 */
export function invariantAvailableInventorySufficient(available, requested, sku) {
    if (available < requested) {
        throw new Error(`INVARIANT VIOLATION: Attempting to reserve ${requested} units of ${sku} but only ${available} available. ` +
 ...
 */
export function validateInventoryInvariants(quantity, reserved) {
 ...
 */
export function validateOrderInvariants(status, progress, pickerId, packerId) {
 ...
 */
export function validateOrderItemInvariants(quantity, pickedQuantity) {
