...
export enum HRNZTaxCode {
  ...
  NSW = 'NSW', // Non-salary/wage earner
}

export enum HRKiwiSaverRate {
  RATE_3 = 'RATE_3',
  RATE_4 = 'RATE_4',
  RATE_6 = 'RATE_6',
  RATE_8 = 'RATE_8',
...
export enum HRDeductionCategory {
  TAX = 'TAX',
  KIWISAVER = 'KIWISAVER',
  ACC = 'ACC',
  UNION = 'UNION',
...
export interface HREmployee {
  ...
  gender: string | null;
  nationalId: string | null; // IRD number
  email: string;
  phone: string | null;
  mobile: string | null;
...
export interface ProcessPayrollInput {
  periodId?: string; // Optional for preview
...
export interface PayrollReportItem {
  ...
  totalCost: number; // grossPay + employerContributions
}

export interface TaxReportQuery {
  taxYear?: string; // YYYY format
  employeeId?: string;
}
