...
export interface SyncProductsRequestDTO {
  connectionId: string;
  skus?: string[]; // Specific SKUs to sync, or all if empty
...
export interface SyncOrdersRequestDTO {
  connectionId: string;
  orderIds?: string[]; // Specific order IDs to sync, or all pending if empty
  daysToLookBack?: number; // Default 7 days
}
