-- Migration: Add order exceptions table
-- Date: 2025-01-18
-- Description: Creates table for tracking order exceptions during fulfillment

-- Order Exceptions table
CREATE TABLE IF NOT EXISTS order_exceptions (
  exception_id VARCHAR(50) PRIMARY KEY,
  order_id VARCHAR(50) NOT NULL REFERENCES orders(order_id) ON DELETE CASCADE,
  order_item_id VARCHAR(50) NOT NULL REFERENCES order_items(order_item_id) ON DELETE CASCADE,
