/**
 * Inbound Receiving Tables Migration
 *
 * Adds support for Advance Shipping Notices (ASN), receipts, and putaway workflows
 */
...
CREATE INDEX idx_asn_line_asn ON asn_line_items(asn_id);
CREATE INDEX idx_asn_line_sku ON asn_line_items(sku);
CREATE INDEX idx_asn_line_status ON asn_line_items(receiving_status);
...
CREATE INDEX idx_receipt_asn ON receipts(asn_id);
...
CREATE INDEX idx_receipt_line_sku ON receipt_line_items(sku);
...
CREATE INDEX idx_putaway_receipt_line ON putaway_tasks(receipt_line_id);
CREATE INDEX idx_putaway_sku ON putaway_tasks(sku);
CREATE INDEX idx_putaway_status ON putaway_tasks(status);
CREATE INDEX idx_putaway_assigned ON putaway_tasks(assigned_to);
CREATE INDEX idx_putaway_target_bin ON putaway_tasks(target_bin_location);
CREATE INDEX idx_putaway_priority ON putaway_tasks(priority);
...
RETURNS TRIGGER AS $$
BEGIN
  -- Update received quantity in ASN line item
...
RETURNS TRIGGER AS $$
BEGIN
  -- Check if all receipt lines are completed
  UPDATE receipts
  SET status = CASE
    WHEN NOT EXISTS (
...
COMMENT ON TABLE advance_shipping_notices IS 'Advance Shipping Notices - expected inbound shipments from suppliers';
COMMENT ON TABLE asn_line_items IS 'Line items for ASNs - detailed list of expected items';
COMMENT ON TABLE receipts IS 'Receipt records - actual received goods from shipments';
COMMENT ON TABLE receipt_line_items IS 'Line items for receipts - detailed received quantities and quality status';
COMMENT ON TABLE putaway_tasks IS 'Putaway tasks - work items for moving received goods to storage locations';
